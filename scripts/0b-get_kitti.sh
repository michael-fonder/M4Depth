...
echo "An account on www.cvlibs.net is required to download the KITTI dataset. By running this script, you acknowledge that you comply to the download policy of www.cvlibs.net (http://www.cvlibs.net/datasets/kitti/user_login.php)."
while true; do
...
if [ ! -d "$SCRIPT_DIR/../datasets/Kitti" ] && [ ! -L "$SCRIPT_DIR/../datasets/Kitti" ]
then
    mkdir -p "$SCRIPT_DIR/../datasets"
    ln -rs "$DB_DIR" "$SCRIPT_DIR/../datasets/Kitti"
fi
...
echo "Downloading: data_depth_annotated"
